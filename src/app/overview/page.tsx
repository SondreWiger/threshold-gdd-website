...
export default function OverviewPage() {
  return (
    <div className="px-6 md:px-12 lg:px-24 py-16 md:py-24">
      ...
      {/* Core Concept */}
      <div className="mb-20">
        <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-4">THE CORE CONCEPT</h3>
        <p className="text-sm text-foreground/75 leading-relaxed max-w-2xl">
          <span className="text-amber/85 font-medium">THRESHOLD</span> weaponizes <span className="text-amber/85 font-medium">tonal dissonance</span>. Players begin as an elite US military squad deployed by <span className="text-amber/85 font-medium">Async Technologies</span> to &quot;secure and contain&quot; the <Link href="/lore" className="text-amber/85 font-medium hover:text-amber transition-colors">Backrooms</Link> — confident, well-equipped, and in control. By the campaign&apos;s midpoint, that confidence is systematically destroyed.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
        <div>
          <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-3">GENRE</h3>
          <p className="text-sm text-foreground/75 font-light">
            <span className="text-amber/75 font-medium">Tactical Survival-Horror</span>, Squad-Based FPS. 1 player (Campaign) / 1–4 (Cooperative Multiplayer).
          </p>
        </div>
        <div>
          <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-3">TARGET PLATFORMS</h3>
          <p className="text-sm text-foreground/75 font-light">
            PC (<span className="text-amber/75 font-medium">Steam/Epic</span>), PlayStation 5, Xbox Series X|S
          </p>
        </div>
      </div>
      ...
      {/* Rating */}
      <div className="mb-20">
        <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-3">RATING & PRICE</h3>
        <p className="text-sm text-foreground/75 font-light max-w-2xl">
          <span className="text-amber/75 font-medium">M (Mature 17+)</span> — $49.99 USD. <span className="text-amber/75 font-medium">Premium single-purchase</span>. No battle pass, no loot boxes, no FOMO mechanics.
        </p>
      </div>

      {/* USPs with visual bars */}
      <div className="mb-20">
        <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-8">UNIQUE SELLING POINTS</h3>
        <div className="space-y-6">
          {[
            ...
            <Link key={usp.num} href={usp.link} className="group flex gap-6 py-4 border-b border-foreground/[0.08] hover:border-amber/10 transition-colors">
              <span className="font-mono text-sm text-foreground/75 shrink-0 group-hover:text-amber/45 transition-colors">{usp.num}</span>
              <div className="flex-1">
                <h4 className="text-[13px] font-medium text-foreground/85 group-hover:text-amber/75 transition-colors mb-1">{usp.title}</h4>
                <p className="text-[12px] text-foreground/75 font-light leading-relaxed max-w-xl">{usp.desc}</p>
              </div>
              <span className="font-mono text-[9px] text-foreground/75 group-hover:text-amber/45 transition-colors shrink-0 mt-1">&rarr;</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      {/* Audience */}
      <div className="mb-20">
        <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-8">TARGET AUDIENCE</h3>
          ...
          {[
            ...
            <div key={a.tier}>
              <div className="flex items-baseline gap-3 mb-2">
                <span className="font-mono text-[9px] text-amber/55 tracking-[0.2em]">{a.tier}</span>
                <span className="font-mono text-[9px] text-foreground/75">AGE {a.age}</span>
              </div>
              <p className="text-[13px] text-foreground/55 font-light leading-relaxed">{a.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Risk Assessment */}
      <div className="mb-20">
        <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-8">RISK ASSESSMENT</h3>
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 py-3 border-b border-foreground/[0.08]">
              <span className="text-[13px] text-foreground/65 flex-1">{r.risk}</span>
              <span className={`font-mono text-[9px] tracking-[0.15em] ${
                r.prob === "HIGH" ? "text-tactical-red/60" :
                r.prob === "MEDIUM" ? "text-amber/55" :
                "text-foreground/55"
              }`}>
                {r.prob}
              </span>
              <span className="text-[12px] text-foreground/65 font-light">{r.mitigation}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Inspirations */}
      <div className="mb-8">
        <h3 className="font-mono text-[10px] text-foreground/65 tracking-[0.2em] mb-6">REFERENCE WORKS</h3>
        <div className="flex flex-wrap gap-x-4 gap-y-2">
          {["Aliens (1986)", "Event Horizon (1997)", "Annihilation (2018)", "SCP Foundation", "Kane Pixels", "GTFO", "Resident Evil 7", "Control", "STALKER", "The Magnus Archives"].map((ref) => (
            <span key={ref} className="text-[12px] text-foreground/65 font-light hover:text-amber/55 transition-colors cursor-default">
              {ref}
            </span>
          ))}
        </div>
